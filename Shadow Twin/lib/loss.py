...
class regularization_loss(_Loss):
    ...
    def forward(self, pred_pose, target_pose, sampled_frame, input_frame, input_vol):
        """
        """
        #print(pred_pose.shape, target_pose.shape)
        ...
        #print('loss_param_r', loss_param_r)
        #print('loss_param_t', loss_param_t)
        loss_img_similarity = 1.0 - self.ms_ssim_loss(sampled_frame*255, input_frame*255)

        # img_sam = (sampled_frame*255)[0,0,0]
        # img_sam = img_sam.to(torch.uint8)
        # img_sam_pil = TF.to_pil_image(img_sam)
        # img_sam_pil.save('/data/shenchengkang/sck/CU-Reg/datasets/SAM.png')

        # img_real = (input_frame*255)[0,0,0]
        # img_real = img_real.to(torch.uint8)
        # img_real_pil = TF.to_pil_image(img_real)
        # img_real_pil.save('/data/shenchengkang/sck/CU-Reg/datasets/REAL.png')
        ...
        transformation_pred = torch.eye(4).unsqueeze(0).repeat(Bs, 1, 1).to('cuda')  # (B,4,4)
        #print(pred_pose.shape)
        pred_rot = tools.axisangle_to_R(pred_pose)
        transformation_pred[:,:3, :3] = pred_rot
        transformation_pred[:,:3, 3] = pred_pose[:,3:]

        transformation_true = torch.eye(4).unsqueeze(0).repeat(Bs, 1, 1).to('cuda')  # (B,4,4)
        target_rot = tools.axisangle_to_R(target_pose)
        transformation_true[:,:3, :3] = target_rot
        transformation_true[:,:3, 3] = target_pose[:,3:]
        loss_param_diss = F.smooth_l1_loss(transformation_pred, transformation_true, reduction='mean')
        #print(loss_param_diss)
        dis_err = self.dis_err(transformation_pred,transformation_true)
        #print(dis_err)
        ncc_err = normalized_cross_correlation(sampled_frame*255, input_frame*255)
        ...
        return loss_param_t, loss_param_r, loss_img_similarity, dis_err, ncc_err
...
class CornerDistLoss(nn.Module):
    ...
    def forward(self, predict, label, device=torch.device("cuda")):
        ...
        corner1 = torch.tensor([-73, -73, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner2 = torch.tensor([-73,  73, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner3 = torch.tensor([ 73, -73, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner4 = torch.tensor([ 73,  73, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
